  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      <View style={styles.chartContainer}>
        ...
        <LineChart data={getLast7DaysProgress()} width={screenWidth - 40} height={220} chartConfig={chartConfig} bezier style={styles.chart} fromZero />
      </View>

      {getCategoryStats().length > 0 && (
        <View style={styles.chartContainer}>
          <Text style={styles.sectionTitle}>Por Categoría</Text>
          <PieChart data={getCategoryStats()} width={screenWidth - 40} height={220} chartConfig={chartConfig} accessor="population" backgroundColor="transparent" paddingLeft="15" absolute style={styles.chart} />
        </View>
      )}
      ...
      <View style={styles.chartContainer}>
        ...
        <BarChart data={getFrequencyStats()} width={screenWidth - 40} height={220} yAxisLabel="" yAxisSuffix="" chartConfig={{ ...chartConfig, color: (opacity = 1) => `rgba(78, 205, 196, ${opacity})` }} style={styles.chart} showValuesOnTopOfBars fromZero />
      </View>
...
const themedStyles = (theme: NavTheme, paperTheme: MD3Theme) => StyleSheet.create({
  ...
  contentContainer: { paddingBottom: TAB_BAR_HEIGHT + 20 },
  title: { fontSize: 28, fontWeight: 'bold', textAlign: 'center', marginVertical: 20, color: theme.colors.text },
  summaryContainer: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-around', paddingHorizontal: 20, marginBottom: 20 },
  summaryCard: { backgroundColor: theme.colors.card, borderRadius: 16, padding: 16, alignItems: 'center', width: '45%', marginBottom: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 3 },
